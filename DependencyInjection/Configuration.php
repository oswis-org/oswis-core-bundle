...
class Configuration implements ConfigurationInterface
{
    ...
    private function addEmailConfig(NodeDefinition $rootNode): void
    {
        $rootNode->children() /// System e-mails settings.
            ...
            'Return-Path address.'
                 ...
                 ->scalarNode('archive_address')->info('Address for archivation.')->defaultValue(null)->example('archive@oswis.org')->end()->scalarNode('archive_name')->info(
                'Name of archive.'
            )->defaultValue(null)->example('OSWIS Archive')->end()->scalarNode('default_subject')->info(
                'Default subject for messages without set subject.'
            )->defaultValue('System message')->example('Message')->end()->scalarNode('logo')->info('Path of logo.')->defaultValue('../public/img/web/logo-whitebg.png')->example(
                '../public/img/web/logo-whitebg.png'
            )->end()->end()->end();
    }
    ...
    private function addWebConfig(NodeDefinition $rootNode): void
    {
        $rootNode->children() /// Web settings.
                ...
                'Simple modular information system based on ApiPlatform.'
            )->example('Personal website of John Doe. Contains some info.')->end()->scalarNode('geo_x')->info('Geo coordinates - x.')->defaultValue('49.5923997')->example(
                '49.5923997'
            )->end()->scalarNode('geo_y')->info('Geo coordinates - y.')->defaultValue('17.2635003')->example('17.2635003')->end()->scalarNode('color')->info('Theme color.')
                 ->defaultValue('#006FAD')->example('#006FAD')->end()->scalarNode('logo')->defaultValue('@images/logo.png')->info('Path to app logo.')->example(
                ['@images/logo.png', '../assets/assets/images/logo.png']
            )->end()->scalarNode('social_image')->defaultValue('@images/logo.png')->info('Path to app logo.')->example(['@images/logo.png', '../assets/assets/images/logo.png'])
                 ...
                 ->scalarNode('copyright')->info('Copyright owner of application.')->defaultValue('Jakub Zak (https://jakubzak.cz)')->example('Jakub Zak (https://jakubzak.cz)')
                 ->end()->end()->end();
    }
    ...
    {
        $rootNode->children()->arrayNode('angular_admin')->info('Settings for angular admin front-end.')->addDefaultsIfNotSet()->children()->arrayNode('scripts')->arrayPrototype()
                 ->children()->scalarNode('name')->end()->scalarNode('options')->end()->end()->end()->end()->arrayNode('styles')->arrayPrototype()->children()->scalarNode('name')
                 ->end()->scalarNode('options')->end()->end()->end()->end()->end()->end()->end();
    }
