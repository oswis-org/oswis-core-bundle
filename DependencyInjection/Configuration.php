...
class Configuration implements ConfigurationInterface
{
    ...
    final public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode->info('Default configuration for core module of OSWIS (One Simple Web IS).')->children()
            ...
            )->scalarNode('url')->info('Base URL of app.')->defaultValue('https://oswis.org')->example('https://oswis.org')->end()->scalarNode('name_short')->info(
            ...
            /// Admin info.
            ->arrayNode('admin')->info('Info about main administrator.')->addDefaultsIfNotSet()->children()->scalarNode('name')->info('Name of main admin.')->defaultValue(null)->example(
                'John Doe'
            ...
            )->defaultValue(null)->example('https://oswis.org')->end()->scalarNode('phone')->info('Phone of administrator.')->defaultValue(null)->example('+000 000 000 000')->end()->end()->end()
            ...
            ->arrayNode('email')->info('Sender of system e-mails.')->addDefaultsIfNotSet()->children()->scalarNode('address')->info('E-mail address of sender.')->defaultValue(
                'oswis@oswis.org'
            )->example('john.doe@example.com')->end()->scalarNode('name')->info('Name of sender.')->defaultValue('OSWIS')->example('John Doe')->end()->scalarNode('reply_path')->info(
                'Return-Path address.'
            )->defaultValue(null)->example('info@oswis.org')->end()->scalarNode('return_path')->info('Address for reply.')->defaultValue(null)->example('webmaster@oswis.org')->end()->scalarNode(
                'archive_address'
            )->info('Address for archivation.')->defaultValue(null)->example('archive@oswis.org')->end()->scalarNode('archive_name')->info('Name of archive.')->defaultValue(null)->example(
                'OSWIS Archive'
            )->end()->scalarNode('default_subject')->info('Default subject for messages without set subject.')->defaultValue('System message')->example('Message')->end()->scalarNode('logo')->info(
                'Path of logo.'
            )->defaultValue('../public/img/web/logo-whitebg.png')->example('../public/img/web/logo-whitebg.png')->end()->end()->end()
            /// App general.
            ->arrayNode('web')->info('Website settings.')->addDefaultsIfNotSet()->children()->scalarNode('title')->info('Title of website.')->defaultValue('One Simple Web IS')->example(
            ...
            )->defaultValue('OSWIS')->example('John\'s web')->end()->scalarNode('title_long')->info('Long (full) title of application.')->defaultValue('One Simple Web IS')->example(
                'John\'s personal website'
            ...
            )->end()->scalarNode('geo')->info('Geo coordinates.')->defaultValue('')->example('49.5923997,17.2635003')->end()->scalarNode('color')->info('Theme color.')->defaultValue(
                '#006FAD'
                ...
                ['@ZakjakubOswisCore/Resources/public/logo.png', '../assets/assets/images/logo.png']
            )->end()->scalarNode('author')->info('Author of application.')->defaultValue('Jakub Zak (https://jakubzak.cz)')->example('Jakub Zak (https://jakubzak.cz)')->end()->scalarNode(
                'copyright'
